...
def image_display(images):
    images = images / 2 + 0.5
    images_as_np = images.numpy() 
    plt.imshow(np.transpose(images_as_np, (1,2,0))) 
    plt.show()  


train_data_iter = iter(train_data_loader)
images, labels = next(train_data_iter)
images_grid =torchvision.utils.make_grid(images[0:30],nrow=5)
image_display(images_grid)
'''
